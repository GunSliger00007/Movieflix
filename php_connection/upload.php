...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $unique_movie_filename = uniqid('movie_', true) . '.' . pathinfo($movie_file['name'], PATHINFO_EXTENSION);
    $unique_cover_filename = uniqid('cover_', true) . '.' . pathinfo($cover_image['name'], PATHINFO_EXTENSION);
    ...
    if ($cover_image['error'] === UPLOAD_ERR_OK) {
        $cover_image_path = $image_file_dir . $unique_cover_filename;
    ...
    $sql = "INSERT INTO movies (title, description, release_date, duration, file_path, cover_image) 
            VALUES ('$title', '$description', '$release_date', '$duration', '$movie_file_path', '$cover_image_path')";
